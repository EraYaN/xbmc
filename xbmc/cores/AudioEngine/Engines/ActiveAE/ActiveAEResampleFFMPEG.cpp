...
int CActiveAEResampleFFMPEG::Resample(uint8_t **dst_buffer, int dst_samples, uint8_t **src_buffer, int src_samples, double ratio)
{
  if (ratio != 1.0)
  {
    if (swr_set_compensation(m_pContext,
                             (int)((dst_samples*ratio-dst_samples)*m_dst_rate/m_src_rate),
                             dst_samples*m_dst_rate/m_src_rate) < 0) // this division will not work for all operands (integer division)
    {
